...
More generally,\\
$(A - \lambda_1 I)w \in \spn{e_1, \ldots, e_{i-1}}$ for all $w \in \spn{e_1, \ldots, e_i}$
...
\end{thm}
\pr{
We work over the algebraic closure $\overline{\F} \supset \F$\\
...
Define $f(x) = (x - \lambda_1) \cdots (x - \lambda_n)$, with $\lambda_i$ the distinct eigenvalues of the $v_i$. Then $f(T) = 0$ since $f$ annihilates every element of $\mathcal{B}$. Now, since we must have $m_T | f$ then $m_T = f$, a product of distinct linear factors.
\end{itemize}\qed
